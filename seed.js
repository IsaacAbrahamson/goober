...
for (let i = 1; i <= NUM_DRIVERS; i++ ) {
  let cityNum = Math.floor(Math.random()*cities.length)
  let {longitude, latitude} = cities[cityNum]  

  let name = randomName()
  let person = {
    name,
    email: `${name.replace(/ /g,".")}@no-email.com`,
    geometry: {
      type: 'Point',
  ...
  // remove conditional logic for multiple drivers per city
  if(!usedCities.includes(cities[cityNum].rank)) {
    usedCities.push(cities[cityNum].rank)
    postToAPI(person)
  } else {
    continue
  }  
...
async function postToAPI(person) {
  try {
